...
static void gtk_rot_knob_class_init(GtkRotKnobClass *class, gpointer class_data)
{
...
static void gtk_rot_knob_init(GtkRotKnob *knob, gpointer g_class)
{
    (void)knob;
    (void)g_class;
}
 ...
 *
 * This function is called when a mouse button is pressed on a digit. This is
 ...
 * decimal separator. To get the amount of change corresponding to the clicked
 * label we can use the INDEX_TO_DELTA[] array. The index to this table is
 * attached to the evtbox. Whether the delta is positive or negative depends on
 * which mouse button triggered the event.
 */
static gboolean on_button_press(GtkWidget *evtbox, GdkEventButton *event,
                                gpointer data)
{
    GtkRotKnob *knob = GTK_ROT_KNOB(data);
 ...
 * @return Always TRUE to prevent further propagation of the event
 *
 * This function is called when the mouse wheel is moved up or down. This is
 * used to increment or decrement the value.
 *
 ...
 * The digit labels are stored in an array that also contains the sign and the
 * decimal separator. To get the amount of change corresponding to the clicked
 * label we can use the INDEX_TO_DELTA[] array. The index is attached to the
 * evtbox. Whether the delta is positive or negative depends on the scroll
 * direction.
 */
static gboolean on_button_scroll(GtkWidget *evtbox, GdkEventScroll *event,
                                 gpointer data)
{
    GtkRotKnob *knob = GTK_ROT_KNOB(data);
    guint idx = GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(evtbox), "index"));
...
GType gtk_rot_knob_get_type()
{
    ...
    if (!gtk_rot_knob_type)
    {
        ...
        gtk_rot_knob_type = g_type_register_static(GTK_TYPE_BOX, "GtkRotKnob",
                                                   &gtk_rot_knob_info, 0);
    }
...
GtkWidget *gtk_rot_knob_new(gdouble min, gdouble max, gdouble val)
{
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        gtk_widget_set_tooltip_text(
            knob->digits[i], _("Use mouse buttons and wheel to change value"));
