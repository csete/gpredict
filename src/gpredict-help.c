...
void gpredict_help_show_txt(const gchar *filename)
{
    ...
    fname = g_strconcat(buff, G_DIR_SEPARATOR_S, "doc", G_DIR_SEPARATOR_S,
                        filename, ".txt", NULL);
    g_free(buff);
#endif
    ...
    if (error != NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR, _("%s: Failed to load %s (%s)"),
    ...
    if (error != NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR, _("%s: Error reading %s (%s)"),
                    __func__, fname, error->message);
    ...
    dialog = gtk_dialog_new_with_buttons(_("Gpredict Info"), GTK_WINDOW(app), 0,
