...
static void     addbut_clicked_cb(GtkButton * button,
                                  GtkSatSelector * selector);
static void     delbut_clicked_cb(GtkButton * button,
                                  GtkSatSelector * selector);
            ...
            if (num > 0)
            {
                ...
                if (g_file_test(filename, G_FILE_TEST_EXISTS))
                {
                    ...
                    sat_log_log(SAT_LOG_LEVEL_DEBUG,
                                _
                                ("%s: Already have module %s. Ask user to confirm..."),
                                __func__,
                                gtk_entry_get_text(GTK_ENTRY(namew)));
...
/**
 * Create module configuration window.
 *
...
/**
 * Create location selector combo box.
 *
...
/**
 * Signal handkler for "sat-activated" signals from the GtkSatSelector.
 *
