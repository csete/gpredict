...
static void create_network(GtkWidget * vbox)
{
    ...
    if (sat_cfg_get_str(SAT_CFG_STR_TLE_SERVER))
        gtk_entry_set_text(GTK_ENTRY(server),
                           sat_cfg_get_str(SAT_CFG_STR_TLE_SERVER));
    ...
    if (sat_cfg_get_str(SAT_CFG_STR_TLE_FILES))
        gtk_entry_set_text(GTK_ENTRY(files),
                           sat_cfg_get_str(SAT_CFG_STR_TLE_FILES));
...
static void create_misc(GtkWidget * vbox)
{
    ...
    gtk_widget_set_tooltip_text(addnew,
                                _
                                ("Note that new satellites will be added to a group called Other"));
...
#if 0
/** Create widgets for local update configuration */
static void create_local(GtkWidget * vbox)
{
    GtkWidget      *label;
...
static void reset_cb(GtkWidget * button, gpointer data)
{
    ...
    /* server */
    gtk_entry_set_text(GTK_ENTRY(server),
                       sat_cfg_get_str_def(SAT_CFG_STR_TLE_SERVER));
    ...
    /* files */
    gtk_entry_set_text(GTK_ENTRY(files),
                       sat_cfg_get_str_def(SAT_CFG_STR_TLE_FILES));
