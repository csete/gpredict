...
static void gtk_sky_glance_init(GtkSkyGlance *skg, gpointer g_class)
{
    (void)g_class;
...
GType gtk_sky_glance_get_type()
{
    ...
    if (!gtk_sky_glance_type)
    {
        ...
        gtk_sky_glance_type = g_type_register_static(
            GTK_TYPE_BOX, "GtkSkyGlance", &gtk_sky_glance_info, 0);
    }
...
static gboolean on_draw(GtkWidget *widget, cairo_t *cr, gpointer data)
{
    ...
    font_desc =
        pango_font_description_from_string(skg->font ? skg->font : "Sans 9");
    ...
    /* Axis label */
    {
        const gchar *axis_text = sat_cfg_get_bool(SAT_CFG_BOOL_USE_LOCAL_TIME)
                                     ? _("TIME")
                                     : _("UTC");
                                  ...
                                  gpointer data)
{
    ...
    if (skypass->pass == NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR,
                    _("%s::%s: Could not retrieve pass_t object"), __FILE__,
                    __func__);
        return TRUE;
    }

    new_pass = copy_pass(skypass->pass);
    sat_log_log(SAT_LOG_LEVEL_DEBUG, _("%s::%s: Showing pass details for %s"),
...
static void get_colors(guint i, guint *bcol, guint *fcol)
{
    ...
    default:
        sat_log_log(SAT_LOG_LEVEL_ERROR,
                    _("%s:%d: Colour index out of valid range (%d)"), __FILE__,
                    __LINE__, i);
        tmp = sat_cfg_get_int(SAT_CFG_INT_SKYATGL_COL_01);
        break;
    }
...
static void create_sat(gpointer key, gpointer value, gpointer data)
{
    ...
    if (passes != NULL)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            {
                sat_log_log(SAT_LOG_LEVEL_ERROR,
                            _("%s:%s: Could not allocate memory."), __FILE__,
                            __func__);
                continue;
            }
...
GtkWidget *gtk_sky_glance_new(GHashTable *sats, qth_t *qth, gdouble ts)
{
    ...
    skg->te =
        skg->ts + sat_cfg_get_int(SAT_CFG_INT_SKYATGL_TIME) * (1.0 / 24.0);
    ...
    g_signal_connect(skg->canvas, "motion-notify-event",
                     G_CALLBACK(on_motion_notify), skg);
    g_signal_connect(skg->canvas, "button-release-event",
                     G_CALLBACK(on_button_release), skg);
    g_signal_connect(skg->canvas, "size-allocate", G_CALLBACK(size_allocate_cb),
                     skg);
    g_signal_connect_after(skg->canvas, "realize",
                           G_CALLBACK(on_canvas_realized), skg);
