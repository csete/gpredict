...
static void
        gtk_sat_tree_class_init (GtkSatTreeClass *class)
{
...
static void
        gtk_sat_tree_destroy (GtkObject *object)
{
        ...
        gtk_sat_tree_new (guint flags)
{
    ...
    column = gtk_tree_view_column_new_with_attributes (_("Catnum"), renderer,
                                                       "text", GTK_SAT_TREE_COL_CATNUM,
                                                       "visible", GTK_SAT_TREE_COL_VIS,
                                                       NULL);
    gtk_tree_view_insert_column (GTK_TREE_VIEW (sat_tree->tree), column, -1);
    ...
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (GTK_SAT_TREE (widget)->swin),
    ...
    gtk_box_pack_start (GTK_BOX (widget), GTK_SAT_TREE (widget)->swin, TRUE, TRUE, 0);
...
static GtkTreeModel *
        create_and_fill_model (guint flags)
{
...
/** \brief Scan .tle file and add satellites to GtkTreeStore.
 ...
 */
static gint
        scan_tle_file (const gchar *path, GtkTreeStore *store, GtkTreeIter *node)
{
    guint         i = 0;
    guint         j;
    GIOChannel   *tlefile;
                               ...
                               gpointer               data)
{
    ...
    GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (sat_tree->tree));
    ...
    gtk_tree_model_get (model, &iter,
                        GTK_SAT_TREE_COL_CATNUM, &catnum,
                        GTK_SAT_TREE_COL_SEL, &toggle_item,
                        -1);
...
/** \brief Select a satellite in the GtkSatTree.
 *  \param sat_tree The GtkSatTree widget.
 *  \param catnum Catalogue number of satellite to be selected.
 */
void
        gtk_sat_tree_select    (GtkSatTree *sat_tree, guint catnum)
{
...
/** \brief Foreach callback for checking and selecting a satellite.
 ...
 */
static gboolean
        check_and_select_sat (GtkTreeModel *model,
                              GtkTreePath  *path,
                              GtkTreeIter  *iter,
                              gpointer      data)
{
    ...
    gtk_tree_model_get (model, iter,
                        GTK_SAT_TREE_COL_CATNUM, &cat2,
                        -1);

    if (cat1 == cat2) {
        /* we have a match */
        gtk_tree_store_set (GTK_TREE_STORE (model), iter,
                            GTK_SAT_TREE_COL_SEL, TRUE,
                            -1);
...
/** \brief Foreach callback for unchecking a satellite.
 *  \param model The GtkTreeModel.
 *  \param path The GtkTreePath of the current item.
 *  \param iter The GtkTreeIter of the current item.
 *  \param data Pointer to the GtkSatTree structure.
 ...
 */
static gboolean
        uncheck_sat (GtkTreeModel *model,
                     GtkTreePath  *path,
                     GtkTreeIter  *iter,
                     gpointer      data)
{
...
/** \brief Get list of selected satellites.
 ...
 */
guint *
        gtk_sat_tree_get_selected (GtkSatTree *sat_tree, gsize *size)
{
    guint i;
    gsize s;
    guint *ret;
...
/** \brief Compare two rows of the GtkSatTree.
...
/** \brief Expand all nodes in the GtkSatTree.
...
/** \brief Collapse all nodes in the GtkSatTree.
 *  \param button The GtkButton that received the signal.
 *  \param tree Pointer to the GtkSatTree widget.
 *
