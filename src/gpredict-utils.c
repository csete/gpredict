 ...
 */
GtkWidget *gpredict_hstock_button(const gchar *stock_id, const gchar *text,
                                  const gchar *tooltip)
{
    GtkWidget *button;
    GtkWidget *image;
 ...
 *
 * @note This great trick has been pointed out by Matthias Clasen, he has done
 * the the same for the filter combo in the new GtkFileChooser ref:
 * gtkfilechooserdefault.c:3151 in Gtk+ 2.5.5
 */
...
void rgb2gdk(guint rgb, GdkColor *color)
{
    ...
    if (color == NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR,
                    _("%s:%s: %s called with color = NULL"), __FILE__, __LINE__,
                    __func__);
        return;
    }
...
void rgba2gdk(guint rgba, GdkColor *color, guint16 *alpha)
{
    ...
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR,
                    _("%s:%s: %s called with alpha = NULL"), __FILE__, __LINE__,
                    __func__);
        return;
    }
...
void gdk2rgb(const GdkColor *color, guint *rgb)
{
    ...
    if (rgb == NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR, _("%s:%s: %s called with rgb = NULL"),
...
void gdk2rgba(const GdkColor *color, guint16 alpha, guint *rgba)
{
    ...
    if (rgba == NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR, _("%s:%s: %s called with rgba = NULL"),
                    __FILE__, __LINE__, __func__);
        return;
    }
 ...
 *
 *  this is a substitute for strcasestr which is a gnu c extension and not
...
gboolean gpredict_save_key_file(GKeyFile *cfgdata, const char *filename)
{
    ...
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR,
                    _("%s: Error writing config data (%s)."), __func__,
...
void rgba_from_cfg(guint cfg_rgba, GdkRGBA *gdk_rgba)
{
    if (gdk_rgba == NULL)
    {
        sat_log_log(SAT_LOG_LEVEL_ERROR, _("%s called with gdk_rgba = NULL"),
                    __func__);
        return;
    }
...
/* Convert a 0xRRGGBB encoded config integer to a GdkRGBA structure (no alpha
 * channel) */
...
/* convert GdkRGBA struct to 0xRRGGBB formatted config integer, ignoring the
 * alpha channel */
