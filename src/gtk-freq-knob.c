...
static const guint idx[] = {0, 2, 3, 4, 6, 7, 8, 10, 11, 12};
                                     ...
                                     gpointer class_init)
{
    ...
    freq_changed_signal =
        g_signal_new("freq-changed", G_TYPE_FROM_CLASS(class),
                     G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION, 0, NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void gtk_freq_knob_init(GtkFreqKnob *knob, gpointer g_class)
{
    (void)g_class;
 ...
 * @param data Pointer to the GtkFreqKnob widget (we need it to update the
 * value)
 *
 ...
 * events though)
 *
 * The digit labels are stored in an array. To get the amount of change
 * corresponding to the clicked label we can convert the index (attached to the
 * evtbox): delta = 10^(9-index)
 *
 * Whether the delta is positive or negative depends on which mouse button
 * triggered the event.
 */
...
GType gtk_freq_knob_get_type()
{
    ...
    if (!gtk_freq_knob_type)
    {
        ...
        gtk_freq_knob_type = g_type_register_static(GTK_TYPE_BOX, "GtkFreqKnob",
...
GtkWidget *gtk_freq_knob_new(gdouble val, gboolean buttons)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        else
        {
            ...
            g_object_set_data(G_OBJECT(knob->buttons[i]), "delta",
                              GINT_TO_POINTER(delta));
            gtk_grid_attach(GTK_GRID(table), knob->buttons[i], idx[i], 0, 1, 1);
            ...
            gtk_button_set_relief(GTK_BUTTON(knob->buttons[i + 10]),
                                  GTK_RELIEF_NONE);
            g_object_set_data(G_OBJECT(knob->buttons[i + 10]), "delta",
                              GINT_TO_POINTER(-delta));
            gtk_grid_attach(GTK_GRID(table), knob->buttons[i + 10], idx[i], 2,
                            1, 1);
    ...
    /* Add misc labels */
    markupBufSize =
        strlen(beginTag) + strlen(endTag) + strlen(thousandsSep) + 1;
    markup = (char *)malloc(sizeof(char) * markupBufSize);
    if (markup && thousandsSep)
    {
        snprintf(markup, markupBufSize, "%s%s%s", beginTag, thousandsSep,
                 endTag);
    }
    else
    {
