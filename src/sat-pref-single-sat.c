...
/** First row where checkboxes are placed */
#define Y0 1
...
/**
 * Create and initialise widgets for the single pass cfg tab.
 *
...
GtkWidget      *sat_pref_single_sat_create(GKeyFile * cfg)
{
    ...
    /* create the table */
    table =
        gtk_table_new((SINGLE_SAT_FIELD_NUMBER + 1) / COLUMNS + 1, COLUMNS,
                      TRUE);
